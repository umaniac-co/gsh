...
enum { GSH_FC_EDITOR_ARGUMENT_CAP = 3 };
                            ...
                            const gsh_builtin_io *io)
{
    ...
    for (visited = 0; visited < GSH_HISTORY_CAP; visited++) {
        ...
        if (entry == NULL || prefix_length < 0 ||
            ...
            (prefix_length != 0 &&
             io->output(io->opaque, STDOUT_FILENO, prefix,
                        (size_t)prefix_length) != 0) ||
            io->output(io->opaque, STDOUT_FILENO, entry, length) != 0 ||
                              ...
                              size_t *command_length)
{
    ...
    for (visited = 0; visited < GSH_HISTORY_CAP; visited++) {
        ...
        memcpy(command + used, entry, length);
        used += length;
        if (event == last) {
            command[used] = '\0';
            *command_length = used;
                         ...
                         size_t capacity, size_t *length)
{
    ...
    memcpy(original, command, *length + 1U);
    memcpy(old, replacement, old_length);
    old[old_length] = '\0';
    ...
    memcpy(command, original, prefix);
    memcpy(command + prefix, separator + 1U, new_length);
    memcpy(command + prefix + new_length, match + old_length, suffix);
