...
int main(void)
{
    gsh_command_cache *cache = malloc(sizeof(*cache));
    gsh_command_cache *scratch = malloc(sizeof(*scratch));
    ...
    if (cache == NULL || scratch == NULL ||
        prepare_fixture(root, first, second, first_probe, second_probe,
                        search_path) == -1) {
    ...
    memcpy(scratch, cache, sizeof(*scratch));
    ...
    free(scratch);
    if (failed) {
        fputs("command cache: invariant test failed\n", stderr);
        return 1;
    }
    puts("command cache: bounded lookup, invalidation, and failover passed");
    return 0;
}
