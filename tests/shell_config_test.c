...
static bool initial_file_has_shell_defaults(const char *home)
{
    static const char async[] = "shell.async_repl.enabled = true";
    static const char completion[] = "shell.completion.enabled = true";
    char path[4096];
    char contents[4096];
    ssize_t count;
    int descriptor;
    ...
    return strstr(contents, async) != NULL &&
           strstr(contents, completion) != NULL;
}
...
static int removed_history_configuration_cases(const char *home,
                                                gsh_shell_config *config)
{
    ...
    static const char invalid[] =
        "config.version = 1\n"
        "shell.history.unlock_ttl = finite\n";

    if (home == NULL || config == NULL) return 1;
    if (write_configuration(home, legacy) == -1 ||
        gsh_config_load(config, home, false) == -1 ||
        !config->history_enabled) return 1;
    if (write_configuration(home, invalid) == -1 ||
        gsh_config_load(config, home, false) != -1) return 1;
    return 0;
}
...
int main(void)
{
    ...
    static const char disabled[] =
        ...
        "shell.completion.enabled = false\n"
        "shell.history.enabled = false\n";
    ...
    if (!config.async_repl_enabled || !config.completion_enabled ||
        gsh_config_load(&config, home, true) == -1 ||
        !config.async_repl_enabled || !config.completion_enabled ||
        ...
        !initial_file_has_shell_defaults(home) ||
    ...
    (void)rmdir(home);
    if (failed) {
        (void)fprintf(stderr, "shell config: shell defaults failed\n");
        return 1;
    }
    (void)puts("shell config: shell defaults and validation passed");
    return 0;
}
