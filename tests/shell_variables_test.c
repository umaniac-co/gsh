                        ...
                        const char *expected)
{
    ...
    if (!found || strcmp(value, expected) != 0) {
        (void)fprintf(stderr, "variables: %s: expected <%s>, got <%s>\n", name,
    ...
    for (index = 0; index < gsh_variables_count(store); index++) {
        ...
        if (separator != NULL &&
            ...
            if (gsh_variables_is_set(store, index) != set ||
                actual != attributes) {
                (void)fprintf(stderr, "variables: state mismatch for %s\n", name);
                return -1;
            }
            return 0;
        }
    }
    (void)fprintf(stderr, "variables: state missing for %s\n", name);
    return -1;
}

static int test_import_and_update(gsh_variable_store *store)
{
    ...
    if (!require(store != NULL)) return 1;
    if (!require(environment[0] != NULL)) return 1;
    if (gsh_variables_import(store, environment) == -1 ||
...
static int test_basic_journal(gsh_variable_store *store,
                              gsh_variable_store *scratch,
                              gsh_variable_journal *journal)
{
    if (!require(store != NULL && scratch != NULL)) return 1;
    if (!require(journal != NULL)) return 1;
...
static int test_scoped_journal(gsh_variable_store *store,
                               gsh_variable_store *scratch,
                               gsh_variable_journal *journal)
{
    if (!require(store != NULL && scratch != NULL)) return 1;
...
static int test_journal_attributes(gsh_variable_store *store,
                                   gsh_variable_store *scratch,
                                   gsh_variable_journal *journal)
{
...
static int test_readonly_and_atomicity(gsh_variable_store *store,
                                       gsh_variable_store *scratch,
                                       gsh_variable_journal *journal)
{
    if (!require(store != NULL && scratch != NULL)) return 1;
...
int main(void)
{
    static gsh_variable_store store_storage;
    static gsh_variable_store scratch_storage;
    static gsh_variable_journal journal_storage;
    gsh_variable_store *store = &store_storage;
    gsh_variable_store *scratch = &scratch_storage;
    gsh_variable_journal *journal = &journal_storage;

    if (!require(store != NULL && scratch != NULL)) return 1;
    if (!require(journal != NULL)) return 1;
    if (test_import_and_update(store) != 0 ||
        test_basic_journal(store, scratch, journal) != 0 ||
        test_scoped_journal(store, scratch, journal) != 0 ||
        test_journal_attributes(store, scratch, journal) != 0 ||
        test_readonly_and_atomicity(store, scratch, journal) != 0) {
        return 1;
    }
    (void)puts("variables: hash arena, attributes, and atomic journal passed");
    return 0;
}
